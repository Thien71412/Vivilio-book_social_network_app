...
export default class UserSearch extends Component{
    ...
    getUserList= () => {
        AsyncStorage.getItem('access_token').then((access_token) => {
            axios.get(`https://vivilio.herokuapp.com/api/search/users?query=${this.state.query}`,
    ...
    renderItem =({item, index}) => {
        return(
                <TouchableOpacity style={styles.item}
                                    onPress={() => {this.props.navigation.navigate('userSearchDetail')
                                                    AsyncStorage.setItem('idUserHolding', JSON.stringify(item.id))}}
                                                        >
                    <Image source={require('../image/profile.png')} style={{height: 50, width: 50}}/>

                    <View style={styles.wrapText}>
                        
                        <Text style={styles.fontSize}>{item.name}</Text>
                        <Text style={{fontSize: 14, color: '#748c94'}}>{item.email}</Text>
                    </View>
        ...
        return(
            <View style={styles.container}>
                ...
                <View style={{paddingTop: 60, alignItems: 'center', justifyContent: 'center'}}>
                    <Text style={{color: '#748c94', }}>Users were found: {Object.keys(this.state.users).length}
                    </Text>
                </View>
